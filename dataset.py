...
class SegmentationDataset(VisionDataset):
    ...
    def __getitem__(self, index: int) -> Any:
        ...
        with open(image_path, "rb") as image_file, open(mask_path,
            ...
            if self.transforms:
                sample = self.transforms(sample["image"], sample["mask"])
            if self.half_tensor:
                return sample[0].half(), sample[1].half()
            else:
                return sample

def get_dataset(image_set, transforms, half_tensor):
